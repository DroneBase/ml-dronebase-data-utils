...
def test_sync():
    data_url = "s3://ml-detectron-test-dataset/data/solar-panel-dataset/train/annotations/"
    data_dir = "solar-panel-dataset-v2/train/annotations/"
    print(f"Data URL = {data_url}")
    print(f"Data Dir = {data_dir}")

    sync_dir(from_dir=data_url, to_dir=data_dir)
    print("Test has write access: {}".format(os.access('.', os.W_OK)))
    print("Test has read access: {}".format(os.access('.', os.R_OK)))
